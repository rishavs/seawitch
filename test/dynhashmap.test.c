...
static Error dynhashmap_of_int_to_string(int64_t len, int64_t i, void* acc, FxString key, void* item) {
...
// static Error dynhashmap_print_int(int64_t len, int64_t i, void* acc, FxString key, void* item) {
//     if (i == 0) printf("[ ");
//     printf("%s = %lli", key.data, *(int64_t*)item);
//     if (i != len - 1) printf(", ");
//     if (i == len - 1) printf(" ]");
...
Error create_int_hashmap_and_get_values(FxString *desc) {
    *desc = fxstring_create("Hashmap: Can create an int hashmap and get its values");
    ...
    DynHashmap* hmap = dynhashmap_create(INT64, sizeof(int64_t), 8);
    int64_t item_size = (int64_t)sizeof(int64_t);   //no overflow check required, as the value will be very small

    err = dynhashmap_set(hmap, "one", &(int64_t){1}); if (!err.ok) return snitch(err.message.data, __LINE__, __FILE__);
    err = dynhashmap_set(hmap, "two", &(int64_t){2}); if (!err.ok) return snitch(err.message.data, __LINE__, __FILE__);
    err = dynhashmap_set(hmap, "three", &(int64_t){3}); if (!err.ok) return snitch(err.message.data, __LINE__, __FILE__);
...
Error handle_invalid_input_for_get_set_on_byte_hashmap(FxString *desc) {
    *desc = fxstring_create("Hashmap: Can handle invalid inputs for get and set on byte hashmap");
    ...
    DynHashmap* hmap = dynhashmap_create(BYTE, (int64_t)sizeof(Byte), 8);
    Byte b1 = 'a', b2 = 'b';
...
Error get_set_remove_from_object_hashmap(FxString *desc) {
    *desc = fxstring_create("Hashmap: Can get, set and remove from an object hashmap");
    ...
    DynHashmap* hmap = dynhashmap_create(OBJECT, (int64_t)sizeof(Point), 8);
    Point p1 = {1, 2}, p2 = {3, 4};
